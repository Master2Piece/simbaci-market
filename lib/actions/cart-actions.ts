...
export const addItemToCart = async (data: CartItem) => {
  try {
    ...
    if (!sessionCartId) throw new Error('Sesi keranjang tidak ditemukan')
    ...
    if (!cart) {
      if (product.stock < 1) throw new Error('Stok item tidak cukup')
      ...
      if (existItem) {
        if (product.stock < existItem.qty + 1)
          throw new Error('Stok item tidak cukup')
      ...
      return {
        ...
        message: `${product.name} ${
          existItem ? 'diperbarui' : 'berhasil ditambahkan'
        } ke keranjang`,
      }
    }
  } catch (error) {
...
export const removeItemFromCart = async (productId: string) => {
  try {
    ...
    if (!exist) throw new Error('Item tidak ditemukan')
